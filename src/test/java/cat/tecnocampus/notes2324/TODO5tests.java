...
public class TODO5tests {
    ...
    void getOneExistingUser() throws Exception {
        mockMvc.perform(get("http://localhost:8080/users/1")
                ...
                .andExpect(jsonPath("$.notes[*].title", containsInAnyOrder(
                    "Spring Boot Introduction", "Spring Data JPA", "Spring Security")));
    }

    // TODO 5.2: implement the test for getting a non existing user. User with id 100
    @Test
    void getOneNonExistingUser() throws Exception {
        mockMvc.perform(get("http://localhost:8080/users/100")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(MockMvcResultMatchers.content().string("User with id: 100 not found"));
    }
}
